...
class RegisterForm(FlaskForm):
    ...
    Submit = SubmitField('Регистрация', validators=[DataRequired()], render_kw={"class": "w-100 mb-2 btn btn-lg rounded-3 btn-primary"})

class AuthForm(FlaskForm):
    Email = StringField('Электронная почта', validators=[DataRequired(), Length(min=1, max=128), Email()])
    Password = PasswordField('Пароль', validators=[DataRequired()])
    Submit = SubmitField('Войти', render_kw={"class": "w-100 mb-2 btn btn-lg rounded-3 btn-primary"})
...
class CourseForm(FlaskForm):
    ...
    Submit = SubmitField('Добавить', render_kw={"class": "w-100 mb-2 btn btn-lg rounded-3 btn-primary"})
